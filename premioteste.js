...
document.getElementById('buscarButton').addEventListener('click', () => {
    ...
    fetch(`https://api.nobelprize.org/v1/prize.json?year=${anoSelecionado}`)
        ...
        .then(dados => {
            ...
            dados.prizes.forEach((premio, index) => {
                ...
                premio.laureates.forEach(laureado => {
                    categoriaDiv.innerHTML += `<p><strong>Laureado:</strong> ${laureado.firstname} ${laureado.surname} <br><strong>Motivação:</strong> ${laureado.motivation || 'Motivação não disponível'}</p>`;
                });

                resultadosDiv.appendChild(categoriaDiv);

                // Adiciona uma linha separadora após cada categoria
                if (index < dados.prizes.length - 1) { // Não adiciona linha após o último prêmio
                    const separator = document.createElement('div');
                    separator.classList.add('separator');
                    resultadosDiv.appendChild(separator);
                }
            });
        })
        .catch(erro => {
